...
function UpdateTraining(){
    ...
    useEffect(() => {
        const fetchUserData = async () => {
          try {
            const response = await fetch(`http://localhost:8000/api/training/order_training/${id}`);
      ...
      const handleUpdate = async () => {
        try {
          const response = await fetch(`http://localhost:8000/api/training/update_training`, {
            method: 'PUT',
            headers: {

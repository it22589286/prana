...
useEffect(() => {
  async function fetchData() {
    try {
      ...
      const filteredUsers = response.data.user.filter(user =>user.role==='customer');
...
const deleteHandler = async (id) => {
  ...
  return (
    <div>
      <p style={{textAlign:'center',paddingTop:'20px',fontSize:'42px',fontStyle:'bold'}}>Customer Details</p>
